...
/// @notice Validates tokens by flash borrowing from the token/<base token> pool on V1.
/// @notice Returns
...
/// @dev 2/ It is possible that the token does not have any pools on V1 therefore we are not able to perform
/// @dev    a flashloan to test the token.
contract TokenValidator is ITokenValidator, IDragonswapCallee, ImmutableState {
    string internal constant FOT_REVERT_STRING = 'FOT';
    // https://github.com/Dragonswap/core/contracts/DragonswapPair.sol#L46
    ...
    function _validate(
        ...
        address pairAddress = DragonswapLibrary.pairFor(this.factoryV1(), token, baseToken);
        ...
        IDragonswapPair pair = IDragonswapPair(pairAddress);
    ...
    // V1 Call
    function dragonswapCall(
        address,
        uint256 amount0,
        uint256,
        bytes calldata data
    ) external view override {
        IDragonswapPair pair = IDragonswapPair(msg.sender);
